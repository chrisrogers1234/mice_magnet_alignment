def file_names(run_number_list):
    ...
    for run in run_number_list:
        ...
        a_file = "/home/cr67/MAUS/work/reco/MAUS-v2.5.1/"+run+"/"+run+"_recon.root"
        file_list.append(a_file)
    return file_list

class Config(object):
    geometry = "geometry_08020/ParentGeometryFile.dat"
    data_dir = "calculated_tms/fc_test/"
    ...
    analyses = [
        {
            ...
            "tof12_cut_low":30.5,
            "tof12_cut_high":32.0,
            "p_bins":[[175.+i*25., 175.+(i+1)*25.] for i in range(2)],
            "reco_files":file_names([8126]),
            "sigma_cut":1.0,
            "chi2_cut":20.,
            "name":"D2: 94.9 A",
            "color":4,
        ...
        },
        {
            ...
            "tof12_cut_low":28.8,
            "tof12_cut_high":29.8,
            "p_bins":[[260.+i*20., 260.+(i+1)*20.] for i in range(2)],
            "reco_files":file_names([8123]),
            "sigma_cut":1.0,
            "chi2_cut":20.,
            "name":"D2: 104.8 A",
            "color":6,
        ...
        },
        {
            ...
            "tof12_cut_low":28.2,
            "tof12_cut_high":28.7,
            "p_bins":[[350.+i*60., 350.+(i+1)*60.] for i in range(1)],
            "reco_files":file_names([8124]),
            "sigma_cut":1.0,
            "chi2_cut":20.,
            "name":"D2: 170.0 A",
            "color":8,
